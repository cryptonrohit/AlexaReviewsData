...
describe("GetAllReviewsByRating tests", () => {
    ...
    it("get_WhenReviewDataInDB_ShouldReturnSuccess", async () => {
        // Arrange
        whereStub.resolves(reviewsTestDataWIthRating2);       
        const expectedResult = { data: reviewsTestDataWIthRating2, totalCount: reviewsTestDataWIthRating2.length, status: Operation.Success };

        // Act
        const actualResult = await getAllReviewsByRating.get(ratingTestData);
